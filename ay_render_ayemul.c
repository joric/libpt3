// uses portions of libauemu: https://github.com/gasman/libayemu
...
static int YM_table [32] = {0, 0, 190, 286, 375, 470, 560, 664, 866, 1130, 1515, 1803, 2253, 2848, 3351, 3862, 4844, 6058, 7290, 8559, 10474, 12878, 15297, 17787, 21500, 26172, 30866, 35676, 42664, 50986, 58842, 65535};
static const int YM_eq[] = {100, 5, 70, 70, 5, 100};
...
int main() {
	...
	ayemu_init(&ay, samplerate, channels, bits);

	fprintf(stderr, "ayemul, ay render, frames: %d, writing wav...\n", frames);
	...
	int frame = 0;
	while (frame < frames) {
		ayemu_set_regs (&ay, (unsigned char*)frame_data[frame]);
	...
	FILE *fp = fopen("out_ayemul.wav", "wb");
