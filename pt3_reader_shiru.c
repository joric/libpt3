...
typedef struct {
	...
	unsigned char Loop_Ornament_Position, Ornament_Length, Position_In_Ornament, Loop_Sample_Position,
		Sample_Length, Position_In_Sample, Volume, Number_Of_Notes_To_Skip, Note, Slide_To_Note, Amplitude;
	bool Envelope_Enabled, Enabled, SimpleGliss;
	short Current_Amplitude_Sliding, Current_Noise_Sliding, Current_Envelope_Sliding, Ton_Slide_Count,
		Current_OnOff, OnOff_Delay, OffOn_Delay, Ton_Slide_Delay, Current_Ton_Sliding, Ton_Accumulator,
...
unsigned char *PT3_FindSig(unsigned char *buffer, long length) {
	...
	char sig1[] = { 0x56, 0x6f, 0x72, 0x74, 0x65, 0x78, 0x20, 0x54, 0x72, 0x61, 0x63, 0x6b, 0x65, 0x72, 0x20,
		0x49, 0x49
	};
...
void PT3_PatternIntterpreter(AYSongInfo * info, PT3_Channel_Parameters * chan, unsigned long chip_num) {
	...
	while (counter > 0) {
		if (counter == flag1) {
			...
			chan->Address_In_Pattern += 2;
			chan->Ton_Delta =
				PT3_GetNoteFreq(info, chan->Note, chip_num) - PT3_GetNoteFreq(info, prnote, chip_num);
...
void PT3_ChangeRegisters(AYSongInfo * info, PT3_Channel_Parameters * chan, char *AddToEnv,
						 unsigned char *TempMixer, unsigned long chip_num) {
	GET_COMMON_VARS(chip_num);
	...
	if (PT3.DelayCounter == 0) {
		...
		if (PT3_A.Note_Skip_Counter == 0) {
			if (module[PT3_A.Address_In_Pattern] == 0) {
				...
				PT3_A.Address_In_Pattern =
					ay_sys_getword(&module
								   [PT3_PatternsPointer + header->PT3_PositionList[PT3.CurrentPosition] * 2]);
				PT3_B.Address_In_Pattern =
					ay_sys_getword(&module
								   [PT3_PatternsPointer + header->PT3_PositionList[PT3.CurrentPosition] * 2 +
									2]);
				PT3_C.Address_In_Pattern =
					ay_sys_getword(&module
								   [PT3_PatternsPointer + header->PT3_PositionList[PT3.CurrentPosition] * 2 +
									4]);
				PT3.Noise_Base = 0;
...
int main() {
	...
	for (;;) {
		...
		info->chip0.reg[13] = 255;	// added by joric
		info->chip1.reg[13] = 255;	// added by joric

		PT3_Play(&AYInfo);

		if (header->PT3_LoopPosition == pos)
			loop = frame;
